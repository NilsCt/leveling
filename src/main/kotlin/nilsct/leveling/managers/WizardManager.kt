...
class WizardManager { // amélioration possible : regrouper en une fonction getEmbed et getActionRows en retournant une seule action
    ...
    private val closedEmbed = Bot.blueEmbed
        .setTitle("That's all!")
        .setDescription("Edit these settings with ${Bot.mention("level-up menu")} and ${Bot.mention("roles menu")}.")
        .build()
    private val cutEmbed = Bot.blueEmbed
        .setTitle("Closed")
        .setDescription(
            """
            This menu has been closed because you opened another one.
            > Open menus with ${Bot.mention("level-up menu")} and ${Bot.mention("roles menu")}
            """.trimIndent()
        )
        .build()
